...
public final class Minecraft implements Runnable {
	...
	public final void run() {
		...
		try {
			...
			Keyboard.create();
			Mouse.create();
			this.mouseHelper = new MouseHelper(this.appletCanvas);
			...
			if(!var24.exists() && !var24.mkdirs()) {
				throw new RuntimeException("The working directory could not be created: " + var24);
			}

			this.mcDataDir = var24;
			...
			IntBuffer var26 = BufferUtils.createIntBuffer(256);
			var26.clear().limit(256);
			this.renderGlobal = new RenderGlobal(this, this.renderEngine);
			GL11.glViewport(0, 0, this.displayWidth, this.displayHeight);
			if(this.serverIp != null && this.session != null) {
				World var27 = new World();
				var27.generate(8, 8, 8, new byte[512]);
				this.setLevel(var27);
		...
		try {
			while(this.running) {
				...
				try {
					...
					for(int var29 = 0; var29 < this.timer.elapsedTicks; ++var29) {
						++this.ticksRan;
						this.runTick();
					}
					...
					if(this.appletCanvas != null && !this.fullScreen && (this.appletCanvas.getWidth() != this.displayWidth || this.appletCanvas.getHeight() != this.displayHeight)) {
						this.displayWidth = this.appletCanvas.getWidth();
						this.displayHeight = this.appletCanvas.getHeight();
				...
				while(System.currentTimeMillis() >= var23 + 1000L) {
					this.debug = var25 + " fps, " + WorldRenderer.chunksUpdated + " chunk updates";
			...
			if(this.objectMouseOver == null) {
				...
				if(this.objectMouseOver.typeOfHit == 1) {
					if(var1 == 0) {
						...
						int var17 = var9 != null ? Item.itemsList[var9.itemID].getIconIndex() : 1;
						if(var17 > 0) {
	...
	public final void setLevel(World var1) {
            this.theWorld = var1;
		try {
			BufferedReader var2 = new BufferedReader(new InputStreamReader((new URL(this.applet.getDocumentBase() + "?n=" + this.session.username + "&i=" + this.session.sessionId)).openStream()));
			Integer.parseInt(var2.readLine());
			var2.close();
			if(this.applet.getDocumentBase().toString().startsWith("http://www.minecraft.net/") || this.applet.getDocumentBase().toString().startsWith("http://minecraft.net/")) {
				this.theWorld = var1;
			}
		...
		if(this.thePlayer == null) {
			...
			this.thePlayer.preparePlayerToSpawn();
			this.playerController.ronRespawn(this.thePlayer);
			if(var1 != null) {
